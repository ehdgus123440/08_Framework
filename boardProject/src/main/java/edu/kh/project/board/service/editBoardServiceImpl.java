...
public class editBoardServiceImpl implements editBoardService {
	...
	// 게시글 삭제
	@Override
	public int boardDelete(int boardNo, Member loginMember) {
		
		int result = 0;
		
		// 게시글 넘버로 글쓴이 조회 후 로그인 멤버와 같으면 삭제
		int writeCheck = mapper.writeCheck(boardNo);
		if(writeCheck == loginMember.getMemberNo()) {
			// 맞으면 게시글 삭제
			result = mapper.boardDelete(boardNo);
		}

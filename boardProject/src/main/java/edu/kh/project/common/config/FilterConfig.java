...
import edu.kh.project.common.filter.BlockFilter;
...
public class FilterConfig {
	...
	@Bean
	public FilterRegistrationBean<BlockFilter> BlockFilter(){
		
		FilterRegistrationBean<BlockFilter> filter 
			= new FilterRegistrationBean<>();
		
		// LoginFilter 객체를 생성해서 filter에 세팅
		BlockFilter BlockFilter = new BlockFilter();
		filter.setFilter(BlockFilter);
		
		// 필터가 동작할 url 패턴 작성
		String[] filteringUrl = {"/*"};
		filter.setUrlPatterns(Arrays.asList(filteringUrl));
		
		filter.setName("BlockFilter");
		filter.setOrder(3);
